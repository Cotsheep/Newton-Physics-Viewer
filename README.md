...
当前版本已经打通桌面资产查看、本地 CPU 摔落冒烟、资产验收入库和浏览器结果查看；正式服务器 GPU 批次仍未开放。README 会把“已经可以运行的功能”和“设计路线”分开说明。
...
| 多高度摔落、坡度试验执行 | 未开放 | 目前只完成就绪检查，不能从菜单运行这些正式试验 |
...
选择菜单第 3 项，程序会列出可运行摔落冒烟的资产。只需选择编号，不需要复制完整 SHA-256 版本号。启动前会显示资产、版本、工况和计算资源摘要，并要求确认。
...
两端都不监听公网或局域网地址，不配置 Tailscale Serve。按 `Ctrl+C` 或关闭控制窗口会结束本次结果访问；浏览服务本身不使用 GPU。统一入口不会自动上传代码、安装依赖、修改服务器 `PATH` 或更新仓库。
